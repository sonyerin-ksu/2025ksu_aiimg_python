...
def show_image(animal_type):
    """선택된 뮤지컬 타입에 맞는 이미지를 로드하여 화면에 표시하는 함수"""
...
dog_button = tk.Button(button_frame, text="무인도", width=10, command=lambda: show_image('무탈'))
dog_button.pack(side=tk.LEFT, padx=5) # 프레임 왼쪽에 배치, 좌우 여백

# 서부 버튼 생성
cat_button = tk.Button(button_frame, text="서부", width=10, command=lambda: show_image('웨스'))
cat_button.pack(side=tk.LEFT, padx=5) # 프레임 왼쪽에 배치, 좌우 여백

# 검은 방 버튼 생성
rabbit_button = tk.Button(button_frame, text="검은 방", width=10, command=lambda: show_image('팬렡'))
rabbit_button.pack(side=tk.LEFT, padx=5) # 프레임 왼쪽에 배치, 좌우 여백

# 이미지를 표시할 레이블 생성
image_label = tk.Label(window, text="버튼을 클릭하여 장소를 선택하세요.", compound=tk.CENTER)
